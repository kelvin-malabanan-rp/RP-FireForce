...
// 3. UPDATE: handleSaveSchedule (single day edit) with loading state
        ...
        for (let day = 1; day <= daysInMonth; day++) {
            ...
            days.push(
                ...
                >
                    ...
                    {eventsStartingHere.map((event, index) => {
                        ...
                        return (
                            <div
                                ...
                                onClick={(e) => {
                                    e.stopPropagation();
                                    handleDateClick(event.startDay);
                                }}
                            >
    ...
    if (error) {
        return (<div className="flex flex-col items-center justify-center h-screen"><AlertCircle className="h-12 w-12 text-red-500 mb-4" /><p className="text-white text-xl mb-2">Error Loading Data</p><p className="text-slate-400 mb-4">{error}</p><Button onClick={loadData} className="bg-blue-600 hover:bg-blue-700"><RefreshCw className="h-4 w-4 mr-2" />Retry</Button></div>);
    }
        ...
        <div className="space-y-6">
            ...
            <Card className="bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700">
                <CardHeader>
                    <div className="flex items-center justify-between">
                        <CardTitle className="flex items-center gap-2 text-slate-900 dark:text-white"><Calendar className="h-5 w-5" />{selectedTeam === 'all' ? 'All Teams Schedule' : selectedTeam === 'my-team' && myTeam ? myTeam.teamName : filteredTeams[0]?.teamName || 'Schedule'}</CardTitle>
                        <div className="flex items-center gap-2">
                            <Button variant="ghost" size="sm" onClick={() => setCurrentDate(new Date(currentDate.getFullYear(), currentDate.getMonth() - 1))} className="text-slate-900 dark:text-white"><ChevronLeft className="h-4 w-4" /></Button>
                            <span className="text-slate-900 dark:text-white font-medium min-w-[150px] text-center">{currentDate.toLocaleDateString('en-US', { month: 'long', year: 'numeric' })}</span>
                            <Button variant="ghost" size="sm" onClick={() => setCurrentDate(new Date(currentDate.getFullYear(), currentDate.getMonth() + 1))} className="text-slate-900 dark:text-white"><ChevronRight className="h-4 w-4" /></Button>
                        </div>
                    </div>
                </CardHeader>
                <CardContent>
                    <div className="grid grid-cols-7 gap-2 mb-4">{['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map((day) => (<div key={day} className="h-10 flex items-center justify-center"><span className="text-sm font-semibold text-slate-600 dark:text-slate-400">{day}</span></div>))}</div>
                    ...
                    <div className="mt-6 flex items-center justify-center gap-6 text-sm border-t border-slate-200 dark:border-slate-700 pt-6">
                        <div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-green-500"></div><span className="text-slate-600 dark:text-slate-400">Primary</span></div>
                        <div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-yellow-500"></div><span className="text-slate-600 dark:text-slate-400">Backup</span></div>
                        <div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-orange-500"></div><span className="text-slate-600 dark:text-slate-400">Escalation</span></div>
                    </div>
                </CardContent>
            </Card>
            {isScheduleModalOpen && selectedDate && (
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={closeModals}>
                    <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-white dark:bg-slate-800 rounded-2xl p-6 border border-slate-200 dark:border-slate-700 max-w-2xl w-full max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
                        <div className="flex items-center justify-between mb-6">
                            <div><h3 className="text-2xl font-bold text-slate-900 dark:text-white">{selectedDate.toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric' })}</h3><p className="text-sm text-slate-600 dark:text-slate-400 mt-1">{selectedTeam === 'all' ? 'All Teams' : filteredTeams[0]?.teamName}</p></div>
                            <Button variant="ghost" size="sm" onClick={closeModals} className="text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white"><X className="h-5 w-5" /></Button>
                        </div>
                        {!isEditMode && (<div className="space-y-4">{filteredTeams.map((team) => {const selectedDateAssignment = team.schedule.find(s => s.date === formatDateKey(selectedDate))?.assignment; return (<div key={team.teamId} className="border border-slate-200 dark:border-slate-700 rounded-lg p-4 bg-slate-50 dark:bg-slate-900/30"><div className="flex items-center justify-between mb-3"><h4 className="font-semibold text-slate-900 dark:text-white">{team.teamName}</h4>{selectedTeam === 'my-team' && (<Button size="sm" onClick={() => openEditModal(team, selectedDateAssignment)} className="bg-blue-600 hover:bg-blue-700 text-white"><Edit2 className="h-3 w-3 mr-1" />Edit</Button>)}</div>{selectedDateAssignment ? (<div className="space-y-3">{selectedDateAssignment.primary && (<div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg p-3"><Badge className="bg-green-500 text-white mb-2">Primary</Badge><div className="flex items-center gap-3"><Avatar className="h-10 w-10 border-2 border-green-500"><AvatarImage src={selectedDateAssignment.primary.avatarUrl} /><AvatarFallback className="bg-green-500 text-white text-xs">{selectedDateAssignment.primary.firstName[0]}{selectedDateAssignment.primary.lastName[0]}</AvatarFallback></Avatar><div><p className="font-medium text-slate-900 dark:text-white text-sm">{selectedDateAssignment.primary.firstName} {selectedDateAssignment.primary.lastName}</p><p className="text-xs text-slate-600 dark:text-slate-400">{selectedDateAssignment.primary.email}</p></div></div></div>)}{selectedDateAssignment.backup && (<div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg p-3"><Badge className="bg-yellow-500 text-white mb-2">Backup</Badge><div className="flex items-center gap-3"><Avatar className="h-10 w-10 border-2 border-yellow-500"><AvatarImage src={selectedDateAssignment.backup.avatarUrl} /><AvatarFallback className="bg-yellow-500 text-white text-xs">{selectedDateAssignment.backup.firstName[0]}{selectedDateAssignment.backup.lastName[0]}</AvatarFallback></Avatar><div><p className="font-medium text-slate-900 dark:text-white text-sm">{selectedDateAssignment.backup.firstName} {selectedDateAssignment.backup.lastName}</p><p className="text-xs text-slate-600 dark:text-slate-400">{selectedDateAssignment.backup.email}</p></div></div></div>)}{selectedDateAssignment.escalation && selectedDateAssignment.escalation.length > 0 && (<div className="bg-orange-50 dark:bg-orange-900/20 border border-orange-200 dark:border-orange-800 rounded-lg p-3"><Badge className="bg-orange-500 text-white mb-2">Escalation</Badge><div className="space-y-2">{selectedDateAssignment.escalation.map((person, idx) => (<div key={idx} className="flex items-center gap-3"><Avatar className="h-8 w-8 border-2 border-orange-500"><AvatarImage src={person.avatarUrl} /><AvatarFallback className="bg-orange-500 text-white text-xs">{person.firstName[0]}{person.lastName[0]}</AvatarFallback></Avatar><div><p className="font-medium text-slate-900 dark:text-white text-sm">{person.firstName} {person.lastName}</p><p className="text-xs text-slate-600 dark:text-slate-400">{person.email}</p></div></div>))}</div></div>)}</div>) : (<div className="text-center py-6"><p className="text-sm text-slate-600 dark:text-slate-400">No schedule set</p>{selectedTeam === 'my-team' && (<Button size="sm" onClick={() => openEditModal(team, null)} className="bg-blue-600 hover:bg-blue-700 text-white mt-3"><Plus className="h-3 w-3 mr-1" />Add</Button>)}</div>)}</div>);})}</div>)}
                        {isEditMode && selectedTeamForEdit && (<div className="space-y-4"><div className="mb-4"><p className="text-sm text-slate-600 dark:text-slate-400">Editing: <span className="font-semibold text-slate-900 dark:text-white">{selectedTeamForEdit.teamName}</span></p></div><div className="space-y-2"><Label className="text-slate-900 dark:text-white flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-green-500"></div>Primary On-Call</Label><div className="space-y-2 max-h-48 overflow-y-auto">{getUsersByRole(selectedTeamForEdit.teamId, 'primary').map(member => {const isSelected = editData.primaryUser === member.id; return (<div key={member.id} onClick={() => setEditData({...editData, primaryUser: isSelected ? '' : member.id})} className={`flex items-center gap-3 p-3 rounded-lg cursor-pointer border-2 transition-all ${isSelected ? 'bg-green-50 dark:bg-green-900/30 border-green-500' : 'bg-slate-50 dark:bg-slate-900/50 border-slate-200 dark:border-slate-700 hover:border-green-300'}`}><div className={`w-5 h-5 rounded border-2 flex items-center justify-center ${isSelected ? 'bg-green-500 border-green-500' : 'border-slate-300 dark:border-slate-600'}`}>{isSelected && <CheckCircle className="h-4 w-4 text-white" />}</div><Avatar className="h-8 w-8"><AvatarImage src={member.avatarUrl} /><AvatarFallback className="text-xs bg-green-500 text-white">{member.firstName[0]}{member.lastName[0]}</AvatarFallback></Avatar><div className="flex-1"><p className="font-medium text-slate-900 dark:text-white text-sm">{member.firstName} {member.lastName}</p><p className="text-xs text-slate-600 dark:text-slate-400">{member.email}</p></div></div>);})}{getUsersByRole(selectedTeamForEdit.teamId, 'primary').length === 0 && (<p className="text-sm text-slate-500 text-center py-4">No primary users available</p>)}</div></div><div className="space-y-2"><Label className="text-slate-900 dark:text-white flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-yellow-500"></div>Backup On-Call</Label><div className="space-y-2 max-h-48 overflow-y-auto">{getUsersByRole(selectedTeamForEdit.teamId, 'backup').map(member => {const isSelected = editData.backupUser === member.id; return (<div key={member.id} onClick={() => setEditData({...editData, backupUser: isSelected ? '' : member.id})} className={`flex items-center gap-3 p-3 rounded-lg cursor-pointer border-2 transition-all ${isSelected ? 'bg-yellow-50 dark:bg-yellow-900/30 border-yellow-500' : 'bg-slate-50 dark:bg-slate-900/50 border-slate-200 dark:border-slate-700 hover:border-yellow-300'}`}><div className={`w-5 h-5 rounded border-2 flex items-center justify-center ${isSelected ? 'bg-yellow-500 border-yellow-500' : 'border-slate-300 dark:border-slate-600'}`}>{isSelected && <CheckCircle className="h-4 w-4 text-white" />}</div><Avatar className="h-8 w-8"><AvatarImage src={member.avatarUrl} /><AvatarFallback className="text-xs bg-yellow-500 text-white">{member.firstName[0]}{member.lastName[0]}</AvatarFallback></Avatar><div className="flex-1"><p className="font-medium text-slate-900 dark:text-white text-sm">{member.firstName} {member.lastName}</p><p className="text-xs text-slate-600 dark:text-slate-400">{member.email}</p></div></div>);})}{getUsersByRole(selectedTeamForEdit.teamId, 'backup').length === 0 && (<p className="text-sm text-slate-500 text-center py-4">No backup users available</p>)}</div></div><div className="space-y-2"><Label className="text-slate-900 dark:text-white flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-orange-500"></div>Escalation Contacts</Label><div className="space-y-2 max-h-48 overflow-y-auto">{getUsersByRole(selectedTeamForEdit.teamId, 'escalation').map(member => {const isSelected = editData.escalationUsers.includes(member.id); return (<div key={member.id} onClick={() => {if (isSelected) {setEditData({...editData, escalationUsers: editData.escalationUsers.filter(id => id !== member.id)});} else {setEditData({...editData, escalationUsers: [...editData.escalationUsers, member.id]});}}} className={`flex items-center gap-3 p-3 rounded-lg cursor-pointer border-2 transition-all ${isSelected ? 'bg-orange-50 dark:bg-orange-900/30 border-orange-500' : 'bg-slate-50 dark:bg-slate-900/50 border-slate-200 dark:border-slate-700 hover:border-orange-300'}`}><div className={`w-5 h-5 rounded border-2 flex items-center justify-center ${isSelected ? 'bg-orange-500 border-orange-500' : 'border-slate-300 dark:border-slate-600'}`}>{isSelected && <CheckCircle className="h-4 w-4 text-white" />}</div><Avatar className="h-8 w-8"><AvatarImage src={member.avatarUrl} /><AvatarFallback className="text-xs bg-orange-500 text-white">{member.firstName[0]}{member.lastName[0]}</AvatarFallback></Avatar><div className="flex-1"><p className="font-medium text-slate-900 dark:text-white text-sm">{member.firstName} {member.lastName}</p><p className="text-xs text-slate-600 dark:text-slate-400">{member.email}</p></div></div>);})}{getUsersByRole(selectedTeamForEdit.teamId, 'escalation').length === 0 && (<p className="text-sm text-slate-500 text-center py-4">No escalation contacts available</p>)}</div></div><div className="flex gap-3 mt-6"><Button onClick={() => setIsEditMode(false)} variant="outline" className="flex-1 border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white">Cancel</Button><Button onClick={handleSaveSchedule} className="flex-1 bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 text-white"><CheckCircle className="mr-2 h-4 w-4" />{!editData.primaryUser && !editData.backupUser && editData.escalationUsers.length === 0 ? 'Clear Schedule' : 'Save Schedule'}</Button></div></div>)}
                    </motion.div>
                </div>
            )}
