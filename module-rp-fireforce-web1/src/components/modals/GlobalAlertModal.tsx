...
export function GlobalAlertModal() {
  ...
  useEffect(() => {
    ...
    // Find ANY unread incident notification
  ...
  const stopSound = () => {
    const audio = alertAudioRef.current;
    if (audio) {
      console.log('🔇 Stopping alert audio');
      audio.pause();
      audio.currentTime = 0;
    }
  };

  // ✅ Close modal helper: closes modal and temporarily suppresses auto-reopen
  const closeModal = (suppressMs = 30_000) => {
    try {
      if (typeof window !== 'undefined') {
        (window as any).suppressGlobalAlertModal = true;
        // clear suppression after suppressMs milliseconds
        setTimeout(() => { try { delete (window as any).suppressGlobalAlertModal; } catch (e) {} }, suppressMs);
      }
    } catch (e) {
      console.warn('Failed to set suppression flag', e);
    }
    stopSound();
    setIsOpen(false);
  };
  ...
  const handleAcknowledge = async () => {
    ...
    if (activeNotification) {
      ...
      setActiveId(null);
      closeModal(); // ✅ Use closeModal instead of setIsOpen(false)
      refresh();
    }
  };
  ...
  return (
      <Dialog
          ...
          onOpenChange={(open) => {
            ...
            if (!open) {
              // ✅ Dialog is closing - stop audio and cleanup
              stopSound();
              if (activeNotification) {
                markAsRead(activeNotification.id);
              }
              ...
              // ✅ Set suppression flag to prevent auto-reopen
              if (typeof window !== 'undefined') {
                (window as any).suppressGlobalAlertModal = true;
                setTimeout(() => {
                  try { delete (window as any).suppressGlobalAlertModal; } catch (e) {}
                }, 30000);
              }
            } else {
              setIsOpen(open);
